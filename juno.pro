...
;               - 0∞ logitude problem for south field line of the satellites
...
; ------------------------------------------------------------------------------
  pro JUNO, spdyn,verbose=verbose,parameters=parameters
...
if ~keyword_set(parameters) then parameters = {JUNO_PARAMETERS}
...
if verbose then message,/info,'Simulation des emissions radio vues par un observateur '
...
tmp=findgen(parameters.simul.nsteps)/float(parameters.simul.nsteps)*parameters.simul.length(0)
  ECRIRE_SPECTRE, spdyn, tmp,'Time (minutes)','',p=parameters
if parameters.output_type.radius eq 1 then $
  ECRIRE_SPECTRE, spdyn, dist_obs,'Distance (Rplanet)','_rs',p=parameters
if parameters.output_type.latitude eq 1 then $
  ECRIRE_SPECTRE, spdyn,lati_obs,'S/C Magnetic Latitude','_lat',p=parameters
if parameters.output_type.local_time eq 1 then $
  ECRIRE_SPECTRE, spdyn, tl_obs,'Local Time (H)','_tl',p=parameters

 ; ECRIRE_SPECTRE, spdyn, parameters.local_time(0), parameters.simul.length(0), *(parameters.freqs.ramp), parameters.output_path,parameters=parameters
set_plot,parameters.display_mgr

if parameters.output_type.diag_theta eq 1 then save,filename="angle.dat",diag_angle
help,parameters.dense.data
print,*(parameters.dense.data)
return
end
