...
sm=s
fm=*(p.freqs.ramp)
if size(sm,/n_dimensions) eq 2 then begin
  sz=size(sm)
  ;if sz(2) gt sz(1) then 
  sm=transpose(reform(sm))
endif else sm=transpose(total(sm, 3))
...
if p.output_type.log eq 1 then spdynps,image,x1,x2,min(U),max(U),xtit,flabel,titre,1,0,0,0,1. else $;,/log
