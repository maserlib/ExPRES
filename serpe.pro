...
fq_b=95
ang_b=fltarr(2,parameters.src.ntot,parameters.simul.nsteps)
...
print, boucle_temps
    FIELD_LINE_EMISSION, stop_flag=stop_flag,parameters,intensite, position,verbose=verbose,theta=diagtheta,ang_b=ang_b,fq_b=fq_b,step_b=boucle_temps
    diag_angle(*,*, boucle_temps)=diagtheta
  endif else begin
    FIELD_LINE_EMISSION, stop_flag=stop_flag,parameters,intensite, position,verbose=verbose,ang_b=ang_b,fq_b=fq_b,step_b=boucle_temps
 endelse
...
sppol=total(spdyn, 3) & w_pol=where(sppol ne 0)
for isrc = 0,parameters.src.ntot-1 do spdyn(*,*,isrc)=spdyn(*,*,isrc)*(*(parameters.src.data))(isrc).pole
if w_pol(0)  ne -1 then sppol(w_pol)=(total(spdyn, 3))(w_pol)/sppol(w_pol)
for isrc = 0,parameters.src.ntot-1 do spdyn(*,*,isrc)=spdyn(*,*,isrc)*(*(parameters.src.data))(isrc).pole

  ECRIRE_SPECTRE, sppol,findgen(parameters.simul.nsteps)/float(parameters.simul.nsteps)*parameters.simul.length(0),$
  'Time (min)','_pol',p=parameters
