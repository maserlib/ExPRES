...
; ====== updating orbiting sources position 
  wsat = where((*p.src.is_orbiting))
  if wsat(0) ne -1 then $
  for boucle_sat=0,total((*p.src.is_orbiting))-1L do begin
      isat = wsat(boucle_sat)
      for boucle_sources=(*p.src.nrange)(0,isat),(*p.src.nrange)(1,isat) do begin
        (*(p.src.data))(boucle_sources).phase_orb = $
             (*(p.src.data))(boucle_sources).phase_orb $
             - p.simul.step*360.*(1./((*(p.src.data))(boucle_sources).distance^1.5*p.planet_param[0])-1./p.planet_param[1]);
        if (*(p.src.data))(boucle_sources).phase_orb ge 360. then (*(p.src.data))(boucle_sources).phase_orb=(*(p.src.data))(boucle_sources).phase_orb-360.
        (*(p.src.data))(boucle_sources).phase_rot += 2.*!pi*p.simul.step*(1./(*(p.src.data))(boucle_sources).period-1./p.planet_param[1])
       endfor
  endfor

; ====== updating sources position (oval local time)
  for boucle_fix=0,n_elements((*p.src.nrange)(0,*))-1L do begin
    for boucle_sources=(*p.src.nrange)(0,boucle_fix),(*p.src.nrange)(1,boucle_fix) do if (*p.src.data)(boucle_sources).fixe_lt then $
    if (*p.src.is_orbiting)(boucle_fix) eq 1 then begin
    (*(p.src.data))(boucle_sources).longitude = (*(p.src.data))(boucle_sources).longitude-$
    p.simul.step*360.*(1./((*(p.src.data))(boucle_sources).distance^1.5*p.planet_param[0])-1./(*(p.src.data))(boucle_sources).period)
    ;+360./(*(p.src.data))(boucle_sources).period*p.simul.step 
