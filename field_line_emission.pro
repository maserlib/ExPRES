...
;wp0  = rebin(reform(wp1,1,ntot),nf,ntot)
sshp= strarr(nf,ntot)
for i_shp=0,nf-1 do sshp(i_shp,*)=shp(1,*)
...
;wp = where((sshp eq "loss_cone"),cntp)

res=STRMATCH(sshp,"*loss*")
wp = where((res eq 1),cntp)
if cntp ne 0 then begin
...
if cntp ne 0 then begin
  ...
  if cnt ne 0 then $
      th(wp(w))=cc/acos(vv(wp(w)))/(180./!pi)*th(wp(w))
endif
...
nu_p = wp0(wp(w))^2/ff(wp(w))^2*nu_c
khi=nu_c/(1.-nu_p)
theta_th=findgen(1000)*0.001*!pi/2.
for i_th=0,cnt-1 DO BEGIN
a=2*nu_p(i_th)/(2.-khi(i_th)*sin(theta_th)^2-sqrt(khi(i_th)^2*sin(theta_th)^4+4.*nu_c(i_th)*cos(theta_th)^2))
...
if res(b) lt 0.1 then th(wp(w(i_th)))=theta_th(b)*180./!pi else th(wp(w(i_th)))=1E4
endfor
;plot,(th(wp(w)))(where(th(wp(w)) ne 1E4))
;oplot,theta2(wp(w))
;;rien=""
;read,rien
endif
