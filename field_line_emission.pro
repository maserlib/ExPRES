...
; ------------------------------------------------------------------------------------
  pro FIELD_LINE_EMISSION, xyz_obs, longitude, pole, chemin, intensite, frequence, $
...
;  endelse
  ...
  lg=longitude*pole+0.5-longi	; longitude ligne de champ (deg)
  if longitude*pole ge 0 then begin
...
;;  if w_1[0] ne -1 then stop,'non interpolated'
...
;  if w[0] ne -1 then stop,'interpolated'
  intensite=fltarr(nf)
