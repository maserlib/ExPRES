...
; ==============================================================================
  pro ADD_SOURCE, parameters=p, name=name, directory=directory, satellite=satellite, $
    ...
    active_lo=active_lo, pole=pole, gradb_test=gradb_test, $
	verbose=verbose
; ==============================================================================
; INPUTS : 
...
if satellite then begin
  ...
  (*p.src.data)(isrc0:isrc1).gradb_test     = keyword_set(gradb_test)
  for isrc = isrc0,isrc1 do for ilon=0,359 do begin
    b  = dblarr(3,nf)
    x  = dblarr(3,nf)
    n = 0L
    file = "./"+(*(p.src.dirs))(iname)+"/"+strtrim(ilon*pole(isrc-p.src.ntot+n_source),2)+".lsr"
